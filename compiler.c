...
static void ifStatement() {
    // if (a) { <-- "a" is a exprssion, push value to stack
    // } else {
    // }
...
static void whileStatement() {
    ...
    expression();
    consume(TOKEN_RIGHT_PAREN, "Expected ')' after while expression");

    int exitJmp = emitJump(OP_JUMP_IF_FALSE);
    emitByte(OP_POP);
    statement();
    emitLoop(loopStart);
