    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_SET_LOCAL: {
                uint8_t slot = READ_BYTE();
                frame->slots[slot] =
                    peek(0); // Don't pop because assignment is an expression;
                break;
            }
            case OP_GET_UPVALUE: {
                uint8_t slot = READ_BYTE();
            ...
            case OP_SET_PROPERTY: {
                ...
                pop(); // Instance
                push(value); // Assign returns the value assigned.
                break;
            }
            ...
            case OP_SET_GLOBAL: {
                ...
                if (tableSet(&vm.globals, name, peek(0))) {
                    ...
                    runtimeError(
                        "Attempt to assign to undefined variable: '%s'",
                        name->chars);
                    return INTERPRET_RUNTIME_ERROR;
                }
                break;
...
InterpretResult interpret(const char* source) {
    ...
    // Push and pop to make the garbage collector aware of the objects.
